...
bikes %>%
  ...
  stat_cor()+
  labs(x = "Feels-like temperature (°F)", y = "Rides",
       title = "Capital Bikeshare: Rides vs. Feels-like Temperature") +
  theme_minimal()
```

```{r}
# A quick, data-driven guess for rides at 75°F
guess_window <- c(70, 80)
rides_guess <- bikes %>%
  filter(dplyr::between(temp_feel, guess_window[1], guess_window[2])) %>%
  summarize(
    n_days = dplyr::n(),
...
# Use the mean of nearby days as a point guess
my_guess_75 <- rides_guess$mean_rides
my_guess_75
```
