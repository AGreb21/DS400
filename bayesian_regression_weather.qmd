...
# Scatter plot matrix
pairs(weather_WU[, c("temp9am", "temp3pm", "humidity9am", "windspeed9am", "pressure9am")], 
      col = as.numeric(weather_WU$location))
```

```{r}
# Key visualizations for understanding the data
# 1. Temperature by location
ggplot(weather_WU, aes(x = temp9am, y = temp3pm, color = location)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Morning vs Afternoon Temperature by Location",
       x = "Temperature at 9am (°C)", 
       y = "Temperature at 3pm (°C)")

# 2. Distribution of temperatures by location
ggplot(weather_WU, aes(x = temp3pm, fill = location)) +
  geom_histogram(alpha = 0.7, bins = 20, position = "identity") +
  facet_wrap(~location) +
  labs(title = "Distribution of 3pm Temperatures by Location",
       x = "Temperature at 3pm (°C)", y = "Count")
```
