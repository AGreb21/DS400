...
```{r}
plot_gamma(shape = 6, rate = 1/2, mean = TRUE, mode = TRUE)
```
...
-   s = 5r + 1
    ...
    -   s / r\^2 = 3
        ...
        -   s = 3r\^2
            ...
            -   5r+1 = 3r\^2
            -   Subtract 5r from both sides: 1 = 3r² - 5r
            -   Subtract 1 from both sides: 0 = 3r² - 5r - 1
            -   **Apply quadratic formula:** r = \[5 ± √(25 + 12)\] / 6
            -   **Simplify:** r = \[5 ± √37\] / 6
            -   **Calculate:** r = \[5 ± 6.08\] / 6
            -   **Two solutions:** r = 11.08/6 ≈ 1.85 or r = -1.08/6 ≈ -0.18
            -   **Keep positive:** r ≈ 1.85
            -   
...
```{r}
plot_gamma(shape = 10.25, rate = 1.85, mean = TRUE, mode = TRUE)
```
...
1.  Why is the Poisson model a reasonable choice for our data Yi?
    ...
    -   This is count data so it fits the data type for the gamma Poisson model
...
```{r}
summarize_gamma_poisson(shape = 1, rate = .25, sum_y = 146, n = 52)
```

```{r}
plot_gamma_poisson(shape = 208, rate = 52, sum_y = 146, n = 52)
```
