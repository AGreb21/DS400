...
ggplot(penguins_complete, aes(x = flipper_length_mm, y = body_mass_g)) +
  ...
  ggpubr::stat_cor(aes(label = ..rr.label..), label.x.npc = "left", label.y.npc = "top") +
...
cv_m1 <- prediction_summary_cv(model = penguin_model_1, data = penguins_complete, k = 10)
cv_m1
...
prediction_summary_cv(model = penguin_model_3, data = penguins_complete, k = 5)
```
...
prediction_summary_cv(model = penguin_model_4, data = penguins_complete, k = 5)
```
...
|------------------|------------------|------------------|------------------|
| 1 | flipper only | 277.8559 | Simple model (1 factor) , relatively effective compared to other models |
| 2 | species only | 334.6865 | Simple model (1 factor), Clearly worse model compared to others |
| 3 | flipper + species | 245.022 | More complex model (2 factors), best model comparitively |
| 4 | all measurements | 275.4792 | Most complex model (All 3 factors), similar effectiveness to model 1 |
...
1.  **Model Selection**: Which model performed best in cross-validation? Was it the most complex model?
    ...
    Model 3 performed the best, with the lowest MAE, not the most complex model which was model 4

2.  **Species vs Measurements**: Does knowing the species add information beyond physical measurements? How can you tell?

    Yes. Comparing Models 1 and 3 shows that adding species on top of flipper length drops MAE by roughly 30 g, and the species coefficients in Model 3 remain far from zero. That means species still explains variation even after accounting for flipper size.

3.  **Practical Use**: If you were a penguin researcher with limited measurement tools, which model would you use and why?

    If I can identify species in the field, I would use Model 3 because it balances accuracy and simplicity. If I only have a tape measure, Model 1 is the next-best option and still yields reasonable errors.

4.  **Prior Sensitivity**: How might different priors have affected your results? Were your priors informative or vague?

    The priors were weakly informative—centered on plausible values but with wide spreads—so the posterior was driven mostly by the data. Tighter priors could shrink extreme slopes or intercepts and widen MAE if they conflict with the sample.

5.  **Assumptions**: What assumptions does normal regression make? Are they reasonable for penguin body mass?

    We assume a linear mean structure, roughly constant variance, and approximately normal residuals. Body mass appears roughly symmetric within species, so these assumptions seem acceptable, though heavier-tailed residuals would inflate predictive uncertainty.
:::
...
fake_news %>%
  ...
  geom_bar(alpha = 0.9) +
  labs(title = "Count of Real vs Fake articles", x = "Type", y = "Count") +
  theme(legend.position = "none")
...
ggplot(fake_news, aes(x = type, y = title_words, fill = type)) +
  ...
  labs(title = "Title length by article type", x = "Type", y = "Title words") +
  theme(legend.position = "none")
...
ggplot(fake_news, aes(x = type, y = negative, fill = type)) +
  geom_boxplot(alpha = 0.9) +
  labs(title = "Negative sentiment by article type", x = "Type", y = "Negative sentiment (0-1)") +
  theme(legend.position = "none")
...
# Ensure outcome is ordered as Fake (0) < Real (1)
...
# Odds ratios with 80% credible intervals
or_ci_80 <- exp(posterior_interval(fake_news_model, prob = 0.80))
or_ci_80

# Tidy odds ratios (median and CI)
library(broom.mixed)
fake_news_or <- tidy(fake_news_model, conf.int = TRUE, conf.level = 0.80, exponentiate = TRUE)
fake_news_or
...
|---------------|---------------|---------------|---------------|---------------|
| 0.3 | 0.700 | 0.978 | 0.283 | Social platforms avoiding false flags on real news |
| 0.5 | 0.733 | 0.922 | 0.450 | Default mix of precision and recall |
| 0.7 | 0.647 | 0.533 | 0.817 | Fact-checkers prioritizing fake detection |
...
fake_news %>%
  add_fitted_draws(fake_news_model, n = 200, scale = "response") %>%
  ggplot(aes(x = title_words, y = .value, group = .draw)) +
  geom_line(alpha = 0.12, color = "#2c7fb8") +
  facet_wrap(~ title_has_excl, labeller = labeller(title_has_excl = c(`TRUE` = "Exclamation in title", `FALSE` = "No exclamation"))) +
...
1.  **Feature Importance**: Which predictor(s) were most strongly associated with fake news? (Hint: look for OR farthest from 1.0) How can you tell from the odds ratios?
    ...
    `title_has_exclTRUE` is the strongest signal, its odds ratio is far below 1, meaning exclamation points drastically reduce the odds of real news. Shorter titles (OR \< 1) and high negative sentiment (OR \< 1) also tilt toward fake stories, but the effect sizes are smaller than the punctuation indicator.
...
2.  **Classification Trade-offs**: In the context of fake news detection, is it worse to:
    ...
    Flagging real news as fake erodes trust, but letting fake news slip through (false positive on real) can misinform the public. If the goal is to rein in misinformation, we should tolerate more manual review of borderline real stories by using a higher cutoff (like 0.7) to raise specificity; if protecting legitimate news is paramount, a lower cutoff (0.3) keeps sensitivity high at the cost of more fake news slipping in.

3.  **Model Limitations**: What information is this model missing? What other predictors might be useful (e.g., source, author, date)?

    We ignore rich text cues such as publisher credibility, topic, writing style, and engagement metrics. Adding features like source domain, sentiment variance, or author history would likely improve discrimination.
...
4.  **Causality Warning**: Can you say that exclamation points **cause** an article to be fake? Why or why not? What's the difference between association and causation?
    ...
    No. We only observe correlations within a static data set, people who write fake news also tend to use exclamation points, but punctuation could be a stylistic side effect rather than the root cause. Causation would require an experiment or a stronger identification strategy.
...
5.  **Real-world Application**: How would you deploy this model in practice? What additional validation would you need before using it to flag articles?
    ...
    I would use this model in a wider range of articles, rather than just this data set, with human checkers/verafiers to improve and confirm its validity before deploying it officially to flag articles.
:::
...
-   [x] Part A: All 7 challenges completed with code and interpretations
-   [x] Part A: Reflection questions answered
-   [x] Part A: Model comparison table completed
-   [x] Part B: All 6 challenges (8-13) completed with code and interpretations
-   [x] Part B: Reflection questions answered
-   [x] Part B: Classification comparison table completed
-   [x] Code is commented and readable
-   [x] All plots have appropriate labels
-   [x] **Rendered to PDF and uploaded to Canvas**
